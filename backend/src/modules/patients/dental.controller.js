...
export const updateDentalChart = async (req, res, next) => {
  try {
    ...
    logActivity({ userId: req.user.sub, userName: req.user.username, action: 'Update Dental Chart', details: `Updated dental chart for patient ${patientId}`, ip: req.ip }).catch(console.error);
