...
import { MedicineCombobox } from "@/modules/prescriptions/components/MedicineCombobox";
import { PatientCombobox } from "@/modules/patients/components/PatientCombobox";
import PrescriptionTemplateModal from "@/modules/prescriptions/components/PrescriptionTemplateModal";
...
const Prescriptions: React.FC = () => {
  ...
  useEffect(() => {
    if (editId) return;
    if (!hasRestored.current) return;  // don't overwrite before restore runs
    const draft = {
      patientId,
      selectedPatient,
      doctorName,
    ...
    localStorage.setItem(DRAFT_KEY, JSON.stringify(draft));
      ...
      chiefComplaint, diagnosis, nextVisitDate, treatmentPlan, templateId, editId]);
              ...
              <div className="mt-4 overflow-hidden rounded-lg border border-border/50">
                ...
                <div className="p-4 space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-medium text-destructive whitespace-pre-wrap">{chiefComplaint || "-"}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm whitespace-pre-wrap">{diagnosis || "-"}</p>
                  </div>
